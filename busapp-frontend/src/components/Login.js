...
function Login() {
  ...
  if (showTimer) clearTimeout(showTimer); // clear old timer if exists
  ...
  try {
    ...
    const bodyText = await res.text(); // read response body ONCE as text

    if (res.ok) {
      const data = JSON.parse(bodyText); // parse text to JSON
      ...
      try {
        const errData = JSON.parse(bodyText); // try parsing error response
        if (errData.message?.toLowerCase().includes('invalid credentials')) {
    ...
    <div className="container py-5" style={{ backgroundColor: '#fdf6e3', minHeight: '100vh' }}>
      ...
      >
        ...
        <p className="mt-4 text-center" style={{ fontSize: '0.95rem' }}>
          ...
          <a href="/register" className="text-warning fw-semibold" style={{ textDecoration: 'underline' }}>
            Register here
          </a>
        </p>

        {message && (
          <p className={`mt-3 text-center fw-semibold ${message.startsWith('Login successful') ? 'text-success' : 'text-danger'}`}>
            {message}
          </p>
        )}
      </div>
